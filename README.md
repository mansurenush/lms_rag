...
## 0) tldr: 
  ...
  реализовал базовый пайплайн, можно было еще много что прикрутить, но ограничение на cpu-only(у меня) вынудили скорее решать задачу не с точки зрения ресерча и экспериментов, а как mvp
...
По умолчанию конфиг ожидает зеркало в `data/offline_docs/501_en/`.
